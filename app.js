...
//This route is off of the 'X' button from the edit Manus page. The 'X' button
//when rendered by the pug has the index of the manufacturer in the list so it 
//can send the index, and this can be spliced out of the array
    ...
    function findItems() {
        if((conditions.stocked == 'before') && (data.name != '') && (data.category != '') && (data.manufacturer != '') && (data.stock_date != '') && (conditions.retail_price_range != ['', '']) && (conditions.buy_price_range != ['', '']) && (conditions.stock_range != ['', '']) && (conditions.minStock_range != ['', ''])&& (conditions.eoq_range != ['', ''])){
            return Item.find({
                ...
                eoq: {$gte: conditions.eoq_range[0],
...
app.get('/:id/restockItems/:mode', async function(req, res){
    ...
    //Getting the id then finding the shop
    const id = req.params.id
    const shop = await Shop.findById(id)
...
app.post("/:id/restockInvManual", async function(req, res){
    ...
    */
    ...
    console.log(req.body['stockVals[]'])
...
app.get('/:id/userhome', async function(req, res){
    ...
    //Getting the id from the params (url), and the user from the db using the id
    const id = req.params.id
...
//user home route for if there is a certain category needed
...
app.post('/:id/editUserAcc', async function(req, res){
    ...
    //If all validation succeeds - THEN
        ...
        User.updateOne(id, updatedUser, async function(err) {
            if(err){
                console.log(err)
            }
            else{
...
app.get('/:userID/shopPage/:shopID/:searchType/:field', async function(req, res){
    ...
    //Getting both user and shop from the DB using the IDs from the url
    const userID = req.params.userID
    const shopID = req.params.shopID
    const user = await User.findById(userID)
    const shop = await Shop.findById(shopID)
...
//Added to cart route. Basically the same as a normal shope page, apart from the fact
...
//This is adding the item to the cart POST route. 
app.post('/:userID/addtoCart/:itemID/shop/:shopID', async function(req, res){

    //Getting the ID's from the url
    const userID = req.params.userID
    const shopID = req.params.shopID
    const itemID = req.params.itemID
    ...
    //Redirecting to the route above, sencding userID, shopID, itemQTY & itemID
...
app.get('/:id/removeItem/:itemIndex', async function(req, res){
    ...
    //Getting the id, the user, and the users cart
    const id = req.params.id
    const user = await User.findById(id)
...
//Adding a bookmark to the bookmarks route
app.get('/:id/bookmarks/addShop/:shopid', async function(req, res){

    //Getting the id of the user from params
    const id = req.params.id
...
app.get('/:id/sales', async function(req, res){
    ...
    //Letting timeframe and transactions as variables
    let timeFrame;
    let transactions;
    ...
    //Instantiating some variables outside the loop so they can be accessed 
    ...
    //Making a rankings array so we can determine 
    let rankings = []
